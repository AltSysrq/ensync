...
use super::compute::{gen_alternate_name, Reconciliation, ReconciliationSide};
use super::context::*;
use crate::defs::*;
use crate::errors::*;
use crate::log;
use crate::log::{ErrorOperation, Log, Logger, ReplicaSide};
use crate::replica::{Condemn, NullTransfer, Replica};
...
fn replace_ancestor<A: Replica + NullTransfer>(
    ...
    fn remove_recursively<A: Replica + NullTransfer>(
        ...
        for (name, fd) in replica.list(dir)? {
            if let FileData::Directory(_) = fd {
                remove_recursively(replica, &mut replica.chdir(dir, &name)?)?;
            }
    ...
    if is_dir(old) && !is_dir(new) {
        remove_recursively(replica, &mut replica.chdir(&*in_dir, name)?)?;
    }
    ...
    match (old, new) {
        ...
        (Some(oldfd), None) => {
            ...
            files.remove(name).expect("Deleted ancestor not in table?");
        }
        (None, Some(newfd)) => {
            replica.create(
                in_dir,
            ...
            files
                .insert(name.to_owned(), newfd.clone())
...
fn try_replace_ancestor<A: Replica + NullTransfer, LOG: Logger>(
    ...
    replace_ancestor(replica, in_dir, files, name, old, new).to_bool(|error| {
        log.log(
            error.level(),
            &Log::Error(
    ...
    match (old, new) {
        ...
        (Some(oldfd), None) => {
            ...
            match dst.remove(dst_dir, File(name, oldfd)) {
                Ok(_) => {
                    dst_files.remove(name).expect("Deleted file not in table?");
                    Ok(None)
                }
                Err(error) => {
                    log.log(
        ...
        (Some(oldfd), Some(newfd)) => {
            ...
            log.log(
                log::EDIT,
                &Log::Update(side, dir_name, name, oldfd, newfd),
            );
            match dst.update(
                dst_dir,
                name,
...
fn try_rename_replica<A: Replica, LOG: Logger>(
    ...
    log.log(log::EDIT, &Log::Rename(side, dir_name, old_name, new_name));
    match replica.rename(dir, old_name, new_name) {
        Ok(_) => {
            let removed =
                files.remove(old_name).expect("Renamed non-existent file?");
        ...
        Err(error) => {
            log.log(
                ...
                &Log::Error(
                    side,
                    dir_name,
                    ErrorOperation::Rename(old_name),
                    &error,
                ),
            );
            false
...
>(
    ...
    dst_files: &mut BTreeMap<OsString, FileData>,
    anc: &ANC,
    anc_dir: &mut ANC::Directory,
    anc_files: &mut BTreeMap<OsString, FileData>,
    src: &SRC,
    src_dir: &SRC::Directory,
    dir_name: &OsStr,
    name: &OsStr,
    old_dst: Option<&FileData>,
    old_src: Option<&FileData>,
    log: &LOG,
    ...
    > Context<CLI, ANC, SRV>
{
    ...
    pub fn apply_reconciliation(
        ...
        old_cli: Option<&FileData>,
        old_srv: Option<&FileData>,
    ) -> ApplyResult {
        ...
        let set_ancestor = match recon {
            ...
            Use(ReconciliationSide::Client) => match apply_use(
                &self.srv,
                &mut dir.srv.dir,
                &mut dir.srv.files,
                &self.anc,
            ...
            Use(ReconciliationSide::Server) => match apply_use(
                &self.cli,
                &mut dir.cli.dir,
                &mut dir.cli.files,
                &self.anc,
            ...
            Split(side, anc_state) => {
                ...
                let ok = match anc_state {
                    ...
                    SplitAncestorState::Move => self
                        .anc
                        .condemn(&mut dir.anc.dir, name)
                        .to_bool(|error| {
                            self.log.log(
                                error.level(),
                                &Log::Error(
                    ...
                    SplitAncestorState::Move => {
                        (!dir.anc.files.contains_key(name)
                            ...
                            && self
                                .anc
                                .uncondemn(&mut dir.anc.dir, name)
                                .to_bool(|error| {
                                    self.log.log(
                                        error.level(),
                                        &Log::Error(
...
pub mod test {
    ...
    use crate::memory_replica::{self, simple_error, DirHandle, MemoryReplica};
    use crate::replica::{NullTransfer, Replica};
    use crate::rules::{
        engine::{DirEngine, FileEngine, SyncRules},
        *,
    };
    ...
    pub fn constant_rules(
        mode: SyncMode,
        trust_client_unix_mode: bool,
    ) -> DirEngine {
        FileEngine::new(Arc::new(SyncRules::single_mode(
            mode,
    ...
    pub type TContext = Context<MemoryReplica, MemoryReplica, MemoryReplica>;
    pub type TDirContext = DirContext<
    ...
    impl Fixture {
        ...
        pub fn context(self) -> TContext {
            Context {
                ...
                root_rules: self
                    .rules
                    .file(File(OsStr::new(""), &FileData::Special)),
                work: WorkStack::new(),
                tasks: UnqueuedTasks::new(),
            }
        }

        pub fn with_context<T, F: FnOnce(&TContext) -> T>(
            &mut self,
            f: F,
        ) -> T {
            let context = Context {
    ...
    pub fn root_dir_context(fx: &Fixture) -> TDirContext {
        DirContext {
    ...
    pub fn mkreg(
        ...
        replica
            .create(dir, File(name, &fd), MemoryReplica::null_transfer(&fd))
            .unwrap();
    ...
    fn apply_recon_insync_creates_ancestor() {
        ...
        let cfd =
            mkspec(&fx.client, &mut root.cli.dir, &mut root.cli.files, &foo);
        let sfd =
            mkspec(&fx.server, &mut root.srv.dir, &mut root.srv.files, &foo);
    ...
    fn apply_recon_use_client_recursive_delete_success() {
        ...
        mkspec(&fx.ancestor, &mut root.anc.dir, &mut root.anc.files, &foo);
        let sfd = mkdir(
            &fx.server,
            &mut root.srv.dir,
            &mut root.srv.files,
        ...
        assert_eq!(
            ApplyResult::RecursiveDelete(ReconciliationSide::Server, 0o777),

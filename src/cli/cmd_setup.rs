...
[[rules.root.files]]
mode = {sync_mode}
# Set to `true` if your filesystem handles UNIX permissions normally, or to
# `false` if it does not (e.g., FAT32, or the `noexec` mount option) and you
...
fn unix_permissions_survive<P: AsRef<Path>>(path: P) -> Result<bool> {
    ...
    // to make the file executable to see the difference.
    //
    // There's also `nosuid`. However, wanting to sync suid bits is rare
    // enough that it's probably not worth auto-detecting. Plus, creating a
    // suid executable as part of a probe is definitely not something we want
    // to be doing here.

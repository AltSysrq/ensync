...
fn ls_one<S: Storage + ?Sized, P: AsRef<Path>>(
    ...
    let mut list = replica.list(&mut dir).chain_err(|| {
        format!("Failed to list '{}'", dir.full_path().to_string_lossy())
    })?;
    ...
    for (name, fd) in list {
        if single.as_ref().map_or(false, |s| *s != name) {
            continue;
        }
        found = true;
        ...
        let (typ, mode, mut size, time, target) = match fd {
            ...
            FileData::Directory(mode) => ('d', mode, 0, 0, None),
...
pub fn mkdir<'a, S: Storage + ?Sized, IT: Iterator<Item = impl AsRef<Path>>>(
    ...
    for path in paths {
        ...
        let (mut dir, _) =
            navigate(replica, path.parent().unwrap_or("".as_ref()), false)?;
        ...
        replica
            .create(&mut dir, File(filename, &FileData::Directory(mode)), None)
...
pub fn get<S: Storage + ?Sized, P1: AsRef<Path>, P2: AsRef<Path>>(
    ...
    fn get_impl<S: Storage + ?Sized>(
        ...
        for (name, fd) in list {
            if single.as_ref().map_or(false, |n| name != *n) {
                continue;
            }
            found = true;
            ...
            match fd {
                FileData::Regular(mode, _, time, _) => {
                    let mut tmpfile =
                        NamedTempFile::new_in(tmpdir).chain_err(|| {
                            format!(
                                "Failed to create \
                                               temporary file"
                            )
                    ...
                    .chain_err(|| {
                        format!("Error transferring '{}'", sub_src.display())
                    })?;

                    posix::set_mtime(&tmpfile, time).chain_err(|| {
                        format!(
                    ...
                    .chain_err(|| {
                        format!("Error persisting '{}'", sub_dst.display())
                    })?;
                }
                ...
                FileData::Directory(mode) => {
                    ...
                    get_impl(
                        replica,
                        replica.chdir(&dir, &name).chain_err(|| {
                            format!(
                ...
                FileData::Symlink(target) => {
                    ...
                    symlink(target, &sub_dst).chain_err(|| {
                        format!(
...
pub fn put<S: Storage + ?Sized, P1: AsRef<Path>, P2: AsRef<Path>>(
    ...
    let name = single
        .or_else(|| src.file_name().map(|s| s.to_owned()))
    ...
    fn put_file<S: Storage + ?Sized>(
        ...
        let md = fs::symlink_metadata(src).chain_err(|| {
        ...
        if ft.is_dir() {
            ...
            .chain_err(|| {
                format!("Failed to create/update '{}'", dst.display())
            })?;

            let mut subdir = replica
                .chdir(&dir, &name)
                .chain_err(|| format!("Failed to enter '{}'", dst.display()))?;
            let sub_existing = map_of(replica, &mut subdir)?;

            let dirit = fs::read_dir(&src).chain_err(|| {
                format!("Failed to list contents of '{}'", src.display())
            })?;
            ...
            let xfer: Option<Box<dyn block_xfer::StreamSource>> =
                Some(Box::new(Xfer(fs::File::open(src).chain_err(|| {
                    format!("Failed to open '{}' for reading", src.display())
                })?)));
            ...
            let target = fs::read_link(src).chain_err(|| {
                format!("Failed to read target of '{}'", src.display())
            })?;
...
fn rm_one<S: Storage + ?Sized, P: AsRef<Path>>(
    ...
    fn remove_by_name<S: Storage + ?Sized>(
        ...
        replica.remove(dir, File(&name, &fd)).chain_err(|| {
            format!(
                "Failed to remove '{}/{}'",
                dir.full_path().to_string_lossy(),
                name.to_string_lossy()
    ...
    fn remove_recursively<S: Storage + ?Sized>(
        ...
        replica.rmdir(&mut dir).chain_err(|| {
            format!("Failed to remove '{}'", dir.full_path().to_string_lossy())
        })
    }
...
fn navigate<S: Storage + ?Sized, P: AsRef<Path>>(
    ...
    path: P,
    allow_tail: bool,
) -> Result<(<ServerReplica<S> as Replica>::Directory, Option<OsString>)> {
    let path = path.as_ref();
    ...
    let mut dir = if path.is_absolute() {
        ...
        replica
            .root()
            .chain_err(|| "Failed to open logical server root")?
    };

    let mut it = path.iter();
    while let Some(component) = it.next() {
        if component.is_empty() || OsStr::new("/") == component {
            continue;
        }
